  ...
  if (newFormSubmissions.length > 0) {
    ...
    for (const newFormSubmission of newFormSubmissions) {
      ...
      if (formSubmission.attachments) {
        console.info("\nCreating submission attachments...\n");

        const attachmentsFolderPath = path.join("./", newFormSubmission.name);

        await filesystem.mkdir(attachmentsFolderPath, {
          recursive: true,
        });
        ...
        for (const attachment of formSubmission.attachments) {
          ...
          await filesystem.writeFile(
            path.join(`./${newFormSubmission.name}`, attachment.name),
            attachmentBuffer,
          );

          console.info(
            `Submission attachment '${attachment.name}' has been created`,
          );
        }

        console.info(
          `Submission attachments are located in folder named '${attachmentsFolderPath}'`,
        );
      }
